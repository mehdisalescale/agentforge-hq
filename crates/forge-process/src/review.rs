//! Code review engine with 6 specialist evaluators and confidence-scored aggregation.
...
mod tests {
    ...
    fn severity_from_confidence_critical() {
        assert_eq!(
            ReviewFinding::severity_from_confidence(95),
            ReviewSeverity::Critical
        );
        assert_eq!(
            ReviewFinding::severity_from_confidence(90),
            ReviewSeverity::Critical
        );
    }
    ...
    fn severity_from_confidence_important() {
        assert_eq!(
            ReviewFinding::severity_from_confidence(85),
            ReviewSeverity::Important
        );
        assert_eq!(
            ReviewFinding::severity_from_confidence(80),
            ReviewSeverity::Important
        );
    }
    ...
    fn severity_from_confidence_minor() {
        assert_eq!(
            ReviewFinding::severity_from_confidence(79),
            ReviewSeverity::Minor
        );
        assert_eq!(
            ReviewFinding::severity_from_confidence(50),
            ReviewSeverity::Minor
        );
    }
    ...
    fn report_filters_below_threshold() {
        let findings = vec![
            ...
            sample_finding(
                ReviewAspect::ErrorHandling,
                85,
                "Swallowed error in handler",
            ),
        ];
